...
imagesize.py
====================
Adapted by EC from https://github.com/scardine/image_size, https://stackoverflow.com/a/19035508
This module analyzes SVG/JPEG/JPEG 2000/PNG/GIF/TIFF/SVG/Netpbm/WebP image headers and returns image size or DIP.

Known bugs: 
- If an image contains an EXIF rotation flag the returned size has to be rotated accordingly
  https://github.com/shibukawa/imagesize_py/issues/49
...
def get_image_size(file_path):
    ...
    """
    img = get_image_metadata_from_path(file_path)
...
def get_image_metadata_from_url(url: str, raise_exc=True) -> Image:

    print("reading image header: " + url)
    ...
    try:
        ...
        # get image metadata
        input = io.BytesIO(r.content)
        size, _ = input.seek(0, 2), input.seek(0)
        return get_image_metadata_from_bytesio(input, size, url)

    except Exception:
        if raise_exc:
            raise
        return Image(path=url, type='UNKNOWN', file_size=0,
                     width=0, height=0)


def get_image_metadata_from_path(file_path):
    """
...
class Test_get_image_size(unittest.TestCase):
    ...
    def test_get_image_metadata(self):
        img = self.data[0]
        output = get_image_metadata_from_path(img['path'])
        self.assertTrue(output)
        for field in image_fields:
...
def main(argv=None):
    ...
    for path_arg in args:
        try:
            img = get_image_metadata_from_path(path_arg)
