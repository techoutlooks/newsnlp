...
class TfidfVectorizer:
    ...
    def __call__(self, raw_documents: [str]) -> [str]:
        ...
        corpus = list(self.tokenizer.corpus)
        self.vocab = Vocab(corpus)
        self.corpus = list(corpus)
        # self.vocab = {str(token) for doc in corpus for token in doc}
        self.N, self.vocab_size = len(corpus), len(self.vocab)

        return self

    def similar_to(self, doc_id, threshold: float = 0.5, top_n: float = 10):
    ...
    def cosine_similarity(self):
        ...
        _ = self._TFIDF / np.linalg.norm(self._TFIDF, axis=1).reshape(self.N, 1)
        return np.dot(_, _.T)
    ...
    def fit_transform(self):
        ...
        """
        tfidf_arr = np.zeros((self.N, self.vocab_size))
